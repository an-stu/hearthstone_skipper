...
升级时只保留 `/Applications/skipper.app` 即可，不要同时运行下载目录或构建目录中的副本。v0.3.5 起支持自动更新：默认启动时检查 GitHub Release，发现新版本后自动下载、校验并安装，完成后自动重启；也可以在菜单栏点击“检查更新”。自动更新可在设置中关闭。
...
4. 可选：启用“在游戏窗口右上角显示一键拔线按钮”。游戏进入全屏后按钮会显示为右上角、背景透明、紧贴文字的小长方形按钮，方便直接点击。首次启用悬浮按钮时，按系统提示授予辅助功能权限。也可以不启用，直接使用菜单栏按钮。
...
**悬浮按钮不显示？** 打开“系统设置 → 隐私与安全性 → 辅助功能”，确认已勾选 HearthStone Skipper。若已勾选仍不显示，说明授权记录对应旧版本：执行 `tccutil reset Accessibility hearthstone-skiper.z2z63.dev`，然后重新启动 Skipper 并按提示重新授权。
